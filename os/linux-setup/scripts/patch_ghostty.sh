#!/bin/bash
# Standalone script: Patch terminal applications to use Ghostty
# This wraps all Terminal=true applications with ghostty

echo "🔧 Patching terminal applications for Ghostty..."
...
grep -rl "Terminal=true" /usr/share/applications 2>/dev/null | while read file; do
    ...
    # Replace Exec to wrap with ghostty
    sed -i "s|^Exec=.*|Exec=ghostty -e ${exec_cmd}|g" "$target"
